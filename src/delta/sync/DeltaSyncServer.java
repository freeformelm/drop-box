...
import sync.FileReader;
import sync.FileWriter;
import sync.PacketBoundedBufferMonitor;
import sync.PacketReceiver;
import sync.PacketSender;

public class DeltaSyncServer {

    public DeltaSyncServer() {
    }
    ...
    public static void receiveFile(PacketBoundedBufferMonitor bm, InetAddress receiverIp, int receiverPort,
        ...
        DeltaFileWriter fileWriter = new DeltaFileWriter(bm, Constants.SERVER_FILE_ROOT);
        fileWriter.start();
        try {
            packetReceiver.join();
            fileWriter.join();
